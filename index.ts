...
class Ingredient {
    ...
    copyWithModifier(modifier: Modifier, newName: string) {
        let ing = new Ingredient(newName, this.image, this.color, this.textColor, this.hooks)
        ing.modifiers = this.modifiers
    ...
    create() {
        ...
        if(!items[this.name as keyof typeof items]){
            items[this.name as keyof typeof items] = this as any
        }
    }
...
class ModifierIngredient extends Ingredient {
    ...
    modifyIngredients(ing: Ingredient[]) {
        ...
        let copy = ing[0].copyWithModifier(this.modifier, `${ModifierNames[this.modifier]} ${ing[0].name}`)
...
const rw = new Ingredient("raspberry water", null, "red", "blue", {onCreate: el => {
...
const garbage = new Ingredient("garbage", null, "green", "white", {onCreate: el => {
    alert(`You created ${el.getAttribute("data-name")}!!!`)
}})

const contamination = new Ingredient("contamination", null, "black", "red", {onCreate: el => {
...
const rd = new Ingredient("raspberry dough", null, "white", "red", {onCreate: el => {
    alert("yummy 😁")
}})

rd.setRecipe(rw, flour)

const rc = new Ingredient("raspberry cookie", null, "brown", "red", {onCreate: el => {
...
craftButton.addEventListener("click", e => {
    ...
    for (let item of usedIngredients) {
        if (item instanceof ModifierIngredient) {
            ...
            items[modified.name as keyof typeof items] = modified as Ingredient & ModifierIngredient
            craftedItems.push(modified)
        }
        if (!item.creates.length) continue
        for (let [ingList, result] of item.creates) {
            if(ingList.length !== usedIngredients.length) continue
            if (ingList.every(v => usedIngredients.includes(v)) && usedIngredients.every(v => ingList.includes(v))) {
